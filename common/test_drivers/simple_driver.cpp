...
void TestDriver::SetUp() {
  if (m_IsReady && !m_IsSet) {
    ...
    hipMalloc(&m_DeviceMatA, m_SizeMatA * m_NumElements * sizeof(real)); CHECK_ERR;
    hipMalloc(&m_DeviceMatB, m_SizeMatB * m_NumElements * sizeof(real)); CHECK_ERR;
    hipMalloc(&m_DeviceMatC, m_SizeMatC * m_NumElements * sizeof(real)); CHECK_ERR;
    m_IsSet = true;
...
void TestDriver::prepareData() {
  if (m_IsSet) {
    ...
    hipMemcpy(m_DeviceMatA, m_HostMatA, m_SizeMatA * m_NumElements * sizeof(real), hipMemcpyHostToDevice);
    CHECK_ERR;
    hipMemcpy(m_DeviceMatB, m_HostMatB, m_SizeMatB * m_NumElements * sizeof(real), hipMemcpyHostToDevice);
    CHECK_ERR;
    hipMemcpy(m_DeviceMatC, m_HostMatC, m_SizeMatC * m_NumElements * sizeof(real), hipMemcpyHostToDevice);
    CHECK_ERR;
  }
  else {
...
std::tuple<real*, real*> TestDriver::getRawResults() {
  if (m_IsSet) {
    hipMemcpy(m_ResultsFromDevice, m_DeviceMatC, m_SizeMatC * m_NumElements * sizeof(real), hipMemcpyDeviceToHost);
    CHECK_ERR;
...
void TestDriver::TearDown() {
  if (m_IsSet) {
    ...
    hipFree(m_DeviceMatA);
    hipFree(m_DeviceMatB);
    hipFree(m_DeviceMatC);
    m_IsSet = false;
  }
  else {
    throw std::string("Test hasn't been set. Call SetUp method first");
