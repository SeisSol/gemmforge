...
generator = CsaGenerator(arch, "float" if args.realsize == 4 else "double")
...
for suite in suites:
    for test in TestLoader(suite):
        mat_a, mat_b, alpha, beta, num_elements, test_name = test

        try:
            generator.generate(mat_a, mat_b, alpha, beta)
